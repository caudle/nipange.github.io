...
class HostAccountWidget extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final _accountBloc = context.read<HostAccountBloc>();
    return Scaffold(
      body: BlocConsumer<HostAccountBloc, HostAccountState>(
                        ...
                        onTap: () {
                          ...
                          Navigator.of(context, rootNavigator: true).push(
                            ...
                            return BlocProvider(
                              ...
                              .read<HostAccountBloc>()
                              .add(
                                  HostAccountEvent.started(snapshot.data!.id)));
                        },
                      ),
                    ),
                    Divider(),
  ...
  Future getDp(String userId, BuildContext context) async {
    ...
    if (pickedFile != null)
      context
          .read<HostAccountBloc>()
          .add(HostAccountEvent.dpChanged(File(pickedFile.path), userId));
    else
...
verifyEmail({
  ...
  if (email.isEmpty) {
    ...
    createEmailDialog(bloc: bloc, context: context).then((value) {
      if (value != null) {
        // add event
        bloc.add(HostAccountEvent.verifyEmail(id: id, email: value));
      }
    });
  } else {
    bloc.add(HostAccountEvent.verifyEmail(id: id, email: email));
  }
}
  ...
  return showDialog(
      ...
      builder: (context) {
        return BlocProvider.value(
          value: bloc,
          child: BlocBuilder<HostAccountBloc, HostAccountState>(
            builder: (context, state) {
              return AlertDialog(
                ...
                content: Form(
                  ...
                  child: TextFormField(
                    ...
                    onChanged: (value) {
                      bloc.add(HostAccountEvent.emailChanged(value));
                    },
...
Future createPhoneDialog({
  ...
  return showDialog(
      ...
      builder: (context) {
        return BlocProvider.value(
          value: bloc,
          child: BlocConsumer<HostAccountBloc, HostAccountState>(
            listener: (context, state) {
                  ...
                  children: [
                    Form(
                      ...
                      child: TextFormField(
                        ...
                        onChanged: (value) {
                          bloc.add(HostAccountEvent.phoneChanged(value));
                        },
                        cursorColor: Theme.of(context).primaryColorDark,
                        validator: (phone) {
                  ...
                  TextButton(
                    onPressed: state.phone.startsWith('0') &&
                        ...
                        ? () {
                            ...
                            bloc.add(
                                HostAccountEvent.validatePhone(state.phone));
                          }
                        : null,
                    child: Text('Verify'),
                    style: ButtonStyle(
...
void onVerificationCompleted({
  ...
  _auth.signInWithCredential(phoneAuthCredential).then((userCredential) {
    // add success event
    bloc.add(HostAccountEvent.phoneVerified(
              ...
              child: Column(
                ...
                children: [
                  TextField(
                    ...
                    onChanged: (value) {
                      bloc.add(HostAccountEvent.otpCodeChanged(value));
                    },
                  ),
                  SizedBox(
                    height: 15,
                  ),
                  BlocBuilder<HostAccountBloc, HostAccountState>(
                    builder: (context, state) {
                      return Text(
  ...
  var timer = Timer.periodic(
    ...
    (timer) {
      if (bloc.state.startTime == 0) {
        timer.cancel();
        bloc.add(HostAccountEvent.startTimeChanged(60, true));
      } else {
        final int time = bloc.state.startTime - 1;
        bloc.add(HostAccountEvent.startTimeChanged(time, false));
      }
    },
  );
  bloc.add(HostAccountEvent.timerChanged(timer));
}
